...
import { getGroupTasks, createGroupTask, completeGroupTask, deleteGroupTask, getUser, type GroupTask } from '../lib/api'
import { PageHeader } from '../components/PageHeader'
import { CheckSquare, Plus, Trash2, Loader2, Check, Star, Lock } from 'lucide-react'
...
export function TasksPage({ userId }: Props) {
  ...
  const { data: tasks, isLoading } = useQuery({
    queryKey: ['grouptasks', userId],
    queryFn: () => getGroupTasks(userId),
  ...
  const createMutation = useMutation({
    mutationFn: () => createGroupTask(userId, form),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['grouptasks', userId] })
      setShowForm(false)
      setForm({ title: '', description: '', pointValue: 10, color: '#7c5cfc' })
    },
  })

  const completeMutation = useMutation({
    mutationFn: (taskId: string) => completeGroupTask(taskId, userId),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['grouptasks', userId] }),
  })

  const deleteMutation = useMutation({
    mutationFn: (taskId: string) => deleteGroupTask(taskId, userId),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['grouptasks', userId] }),
  })
  ...
  return (
    <div className="px-4 pt-12 pb-4 space-y-5">
      <div className="flex items-center justify-between">
        <PageHeader title="Tarefas do Grupo" subtitle="Criadas pelo admin, para todos" Icon={CheckSquare} iconColor="text-primary" />
      </div>
      ...
      {!isAdmin && (
        <div className="flex items-center gap-2 px-4 py-3 rounded-xl"
          ...
          <span className="text-xs text-gray-600">Apenas o criador do grupo pode adicionar ou remover tarefas</span>
        </div>
      )}
      ...
      {earnedToday > 0 && (
        <div className="flex items-center gap-2 px-4 py-3 rounded-xl"
          ...
          <span className="text-sm text-primary font-semibold">
            {earnedToday} / {totalPossible} pts ganhos hoje
          </span>
        </div>
      )}
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          {tasks?.map((task: GroupTask) => (
            <div key={task.id} className="flex items-center gap-3 p-4 rounded-2xl transition-all"
              style={{
                background: task.completedToday ? task.color + '12' : '#1c1c28',
                border: `1px solid ${task.completedToday ? task.color + '35' : '#2a2a3a'}`,
              }}>
              <button
              ...
              <div className="flex-1 min-w-0">
                <p className={`font-semibold text-sm ${task.completedToday ? 'line-through text-gray-600' : 'text-white'}`}>
                  {task.title}
                </p>
          ...
          {tasks?.length === 0 && !showForm && (
            <div className="text-center py-10 text-gray-600">
              ...
              <p className="text-sm">{isAdmin ? 'Crie a primeira tarefa do grupo' : 'Nenhuma tarefa criada ainda'}</p>
            </div>
          )}
        </div>
      )}
      ...
      {isAdmin && (
        showForm ? (
          <div className="rounded-2xl p-4 space-y-4" style={{ background: '#1c1c28', border: '1px solid #2a2a3a' }}>
            <h3 className="font-bold text-white text-sm">Nova tarefa do grupo</h3>
            ...
            <input type="text" placeholder="Descrição (opcional)" value={form.description}
              ...
              style={{ background: '#242434', border: '1px solid #2a2a3a' }} />
            <div>
              <p className="text-xs text-gray-500 mb-2 font-medium">Pontos</p>
              <div className="flex gap-2 flex-wrap">
                {POINT_OPTIONS.map(p => (
                  <button key={p} onClick={() => setForm(f => ({ ...f, pointValue: p }))}
                    className="px-3 py-1.5 rounded-lg text-xs font-bold transition-all"
                    style={{ background: form.pointValue === p ? '#7c5cfc' : '#242434', color: form.pointValue === p ? '#fff' : '#6b7280' }}>
                    {p}
                  </button>
                ))}
              </div>
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => createMutation.mutate()}
                ...
                {createMutation.isPending ? <Loader2 size={14} className="animate-spin mx-auto" /> : 'Criar para todos'}
              </button>
            </div>
          </div>
        ) : (
